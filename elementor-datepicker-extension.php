...
class ElementorDatepickerExtension {
	...
	private $format;
	private $time24;
	private $field_id = "form-field-reservation_date";
	private $min_date = "tomorrow"; // kan ook "today" zijn
	private $max_date = null;
	private $closed_days = array(1,2); //dicht op maandag en dinsdag
	...
	public function init() {
		...
		$this->time24 = apply_filters( 'elementor/datepicker/24h', true ) ? 'true' : 'false';
		...
		add_action( 'elementor_pro/forms/validation/date', array( $this, 'validate_date' ), 10, 3 );
	}
	...
	public function validate_date( $field, $record, $ajax_handler ) {
			// make sure ts the correct date field
			if ( $this->field_id === $field['id'] ) {
				return;
			}
			...
			if($this->min_date != null) {
				$min_allowed_date = strtotime( $this->min_date );
				if ( $min_allowed_date > $form_date ) {
					$ajax_handler->add_error( $field['id'], 'De minimale datum is: ' . date( 'd-m-Y', $min_allowed_date ) );
				}
			}
			
			if($this->max_date != null) {
				$max_allowed_date = strtotime( $this->max_date );
				if ( $max_allowed_date < $form_date ) {
					$ajax_handler->add_error( $field['id'], 'De maximale datum is: ' . date( 'd-m-Y', $max_allowed_date ) );
				}
			}
			
			if($this->closed_days != null) {
				$weekday = date("w", $form_date);
				if(in_array($weekday, $this->closed_days)){
					$ajax_handler->add_error( $field['id'], 'We zijn dicht op deze dag' );
				}
			}
		}
}
